		...
		build: function () {
			...
			if (self.options.wrapper.attr("height")) {
				self.options.wrapper.height(
					self.options.wrapper.attr("height")
				);
			}
...
(function (theme, $) {
	...
	PluginMultiSelect.defaults = {
		templates: {
			li:
				'<li><a class="dropdown-item" tabindex="0"><label style="display: block;"></label></a></li>',
			filter:
		...
		build: function () {
			...
			this.$output.on("change", function () {
				if (self.options.half_values) {
					...
					$($(this).parent().data("plugin-slider-link-values-to"))
						.find(".min")
						.text(values[0]);
					$($(this).parent().data("plugin-slider-link-values-to"))
						.find(".max")
